...
app.get('/:query?', async function(req, res) {
    ...
    await getData(req.params.query || 'any', Math.ceil(Math.random() * 15))
...
async function getData(query, newUrl = 0) {
    await axios({
        ...
        .then((response) => {
            ...
            $('div .concepts .concept_light').each(function(x, elem) {
                word = { furigana: [], text: [], meaning: '' }

                //Adicionando cada conjunto de caracteres FURIGANA da palavra
                $('.furigana span', elem).each((x, elem) => {
                    word.furigana.push($(elem).text())
                })

                //Pega o texto, remove espaços em branco, e separa cada caractere em um array
                word.text = $('.text', elem).text().trim().split('')

                //Pegando tradução da palavra
                translation = $('.meaning-meaning', elem).first().text()
                word.meaning = translation.replace(/\s+/g, ' ').trim()


                //Preenchendo as informações da palavra no array de objetos
                myData.push(word)
            })
            newUrl = "https:" + ($('.more').attr('href'))
